...
class Warehouse:
    ...
    def ware_house_goods_name(self, product: List) -> List[str]:
    ...
    of altenative goods: ")
                    for i in range(len(name_ware_house)):
                        if edit_goods_name in self.pro[i][0]:
                            self.pro[i][1] = str(int(self.pro[i][1])-int(edit_goods_number))# noqa
...
class Opration:
        ...
        for i in range(len(self.product)):
            for j in range(len(self.goods_list)):
                if self.product[i][0] == self.goods_list[j][0]:
                    self.product[i][1] = str(int(self.product[i][1]) - int(self.goods_list[j][1]))# noqa
        return self.product
    ...
    replace with first Item: "))
        alt_list = list()
        alt_list = [self.goods_list[second_Item - 1], self.goods_list[first_Item - 1]]# noqa
...
class Accountants:
    ...
    def total_cost(self) -> str:
        ...
        for i in range(len(self.goods_list)):
            total_costt = int(self.number_goods_list[i]) * int(self.cost_goods_list[i])# noqa
